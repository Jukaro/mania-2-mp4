...
public static class ScoreMetrics {
	...
	public static float ComputeV2Accuracy(ReplayData r) {
		int totalHits = r.NbMax300s + r.Nb300s + r.Nb200s + r.Nb100s + r.Nb50s + r.NbMiss;
		float accuracy = r.NbMax300s * 305 + r.Nb300s * 300 + r.Nb200s * 200 + r.Nb100s * 100 + r.Nb50s * 50;
		float maxAccuracy = totalHits * 305;
		return accuracy / maxAccuracy * 100.0f;
	}

	public static float ComputeLazerV2Accuracy(ReplayData r) {
		int totalHits = r.NbMax300s + r.Nb300s + r.Nb200s + r.Nb100s + r.Nb50s + r.NbMiss;
	...
	public static double ComputePerformancePoints(ReplayData replay, BeatmapDataFromDB beatmap) {
		...
		double starRating = beatmap.ManiaStarRating == null || !beatmap.ManiaStarRating.ContainsKey((int)Mods.None) ? 0.0f : beatmap.ManiaStarRating[(int)Mods.None];
		...
		performancePoints = 8.0 * Math.Pow(Math.Max(starRating - 0.15, 0.05), 2.2) // Star rating to pp curve
								* Math.Max(0, 5 * (ComputeLazerV2Accuracy(replay) / 100) - 4) // From 80% accuracy, 1/20th of total pp is awarded per additional 1% accuracy
