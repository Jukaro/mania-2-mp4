...
public class Menu {
	...
	public void Init() {
		...
		for (int i = 0; i < 10; i++) {
			_buttonContainer.Add(new(_graphics, new(0, 0), "michel" + i, buttonVisualsList[0]));
			_buttonContainer[i].SetColor(new Color(Math.Min(255, i * 20), 0, 0));
		}

		_buttonContainer[2] = new Button(_graphics, new(0, 0), "michel2", buttonVisualsList[1]);
		_buttonContainer[3] = new SliderButton(_graphics, new(0, 0), "Slider", 0, 30, 10, (double d) => Logger.LogDebug($"value: {d}"), buttonVisualsList[1]);
		_buttonContainer[3].ButtonVisuals.BlinkOnMouseClick = false;
		_buttonContainer[3].ButtonVisuals.BlinkOnMouseOver = false;

		_buttonContainer[4] = new ToggleButton(_graphics, new(0, 0), "Play", Play, Pause, buttonVisualsList[0]);
